...
while True:
    ...
    if hand_landmarks:
        ...
        for handLMs in hand_landmarks:
            ...
            mp_drawing_util.draw_landmarks(frame, handLMs, mediapipe_hands.HAND_CONNECTIONS)
            ...
            label_pos = (x_min, y_min - 10)  # Place the label above the rectangle

            # Draw the rectangle and the label on the image
            cv2.rectangle(frame, (x_min, y_min), (x_max, y_max), (0, 255, 0), 2)
            cv2.putText(frame, label_text, label_pos, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
    ...
    if key_press%256 == 27:
        ...
        print(f'before passing to model, analysisframe shape is {image_batch_size_1.shape}')
        ...
        print(f'prediction is {predicted_class} with probability {probability}')
        hand_counter = 0
